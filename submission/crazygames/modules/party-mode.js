...
export function startPartyGame(ctx) {
    const startLogic = () => {
        ...
        const maxPartyPlayers = Number(ctx.maxPartyPlayers) || 20;
        ctx.partyPlayersLeft = Math.min(Math.max(parseInt(count, 10) || 2, 2), maxPartyPlayers);
...
export function handlePartyLetterClick(ctx, letter, btn) {
    ...
    // Se todas as letras foram usadas e ainda há mais de 1 jogador ativo,
    // reinicia o alfabeto mantendo a mesma categoria.
    if (ctx.partyUsedLetters.size >= 26 && ctx.partyPlayersLeft > 1) {
        ctx.partyUsedLetters.clear();
        renderPartyKeyboard(ctx);
        document.getElementById('party-used-count').textContent = '0';
        if (typeof ctx.showFloatingMessage === 'function') {
            ctx.showFloatingMessage('Todas as letras foram usadas! Reiniciando o alfabeto.', 'info');
        }
    }
